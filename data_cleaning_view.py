...
from data_cleaning_manager import make_cleaning
from data_processing_manager import featureclclick
from ipywidgets import *
import settings

def featurecl_click(event):  
    global trgcl_lbl,trgtyp_lbl,miss_lbl
    
    featureclclick(trgcl_lbl,settings.featurescl,trgtyp_lbl,miss_lbl)

    return

def makecleaning(event):
    global result2aexp,missacts
    make_cleaning(settings.featurescl,result2aexp,missacts,settings.dt_features) 

    return

def get_data_cleaning_tab():
    ...
    settings.featurescl.observe(featurecl_click, 'value')
    ...
    mssapplybtn.on_click(makecleaning)
